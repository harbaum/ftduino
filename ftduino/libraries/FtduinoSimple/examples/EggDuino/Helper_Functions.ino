void initHardware(){
  ...
  penState=penDownPos;
	...
	rotMotor.setAcceleration(1000.0);
  ...
  // make sure key is not blocked
  if(ftduino.input_get(Ftduino::I1)) {
    penMotor.move(-100);
    while(penMotor.run());    
  }

  // if input is still blocked something is broken
  if(ftduino.input_get(Ftduino::I1)) {
    while(true) {
  ...
  penMotor.move(1000);
  penMotor.setSpeed(200.0);
  while(!ftduino.input_get(Ftduino::I1) && penMotor.distanceToGo())
    penMotor.runSpeed();

  if(!penMotor.distanceToGo()) {
    while(true) {
      digitalWrite(LED_BUILTIN, LOW);
      delay(100);
      digitalWrite(LED_BUILTIN, HIGH);
...
void servo(char x) {
  Wire.beginTransmission(servoAddr);
  Wire.write(0x00);          // Register 0: pwm value of servo 0
  Wire.write(x);
...
void motorsOff() {
  ...
  // all four motor outputs tristate
...
void motorsOn() {
  ...
  // no need to turn motors on. Moving them will turn them on
  // ...
	motorsEnabled = 1;
}
...
void prepareMove(uint16_t duration, int penStepsEBB, int rotStepsEBB) {
    ...
    displayPos(rotStepsToGo, penStepsToGo);
	}
}
