/*
 Senso/Simon says - ftDuino port - http://ftduino.de
...
#define OUTPUT_RED     Ftduino::O3
#define OUTPUT_GREEN   Ftduino::O4
#define OUTPUT_BLUE    Ftduino::O5
#define OUTPUT_YELLOW  Ftduino::O6

// Button pin definitions
#define INPUT_RED      Ftduino::I1
#define INPUT_GREEN    Ftduino::I2
#define INPUT_BLUE     Ftduino::I3
#define INPUT_YELLOW   Ftduino::I4

// SPEAKER_PIN pin definition
...
void setLEDs(byte leds)
{
  if ((leds & CHOICE_RED) != 0)
    ftduino.output_set(OUTPUT_RED, Ftduino::HI);
  else
    ftduino.output_set(OUTPUT_RED, Ftduino::LO);

  if ((leds & CHOICE_GREEN) != 0)
    ftduino.output_set(OUTPUT_GREEN, Ftduino::HI);
  else
    ftduino.output_set(OUTPUT_GREEN, Ftduino::LO);

  if ((leds & CHOICE_BLUE) != 0)
    ftduino.output_set(OUTPUT_BLUE, Ftduino::HI);
  else
    ftduino.output_set(OUTPUT_BLUE, Ftduino::LO);

  if ((leds & CHOICE_YELLOW) != 0)
    ftduino.output_set(OUTPUT_YELLOW, Ftduino::HI);
  else
    ftduino.output_set(OUTPUT_YELLOW, Ftduino::LO);
}
...
{
  if (ftduino.input_get(INPUT_RED)) return(CHOICE_RED); 
  else if (ftduino.input_get(INPUT_GREEN)) return(CHOICE_GREEN); 
  else if (ftduino.input_get(INPUT_BLUE)) return(CHOICE_BLUE); 
  else if (ftduino.input_get(INPUT_YELLOW)) return(CHOICE_YELLOW);
...
// Toggle SPEAKER_PIN every buzz_delay_us, for a duration of buzz_length_ms.
// TODO: use arduino tone instead
